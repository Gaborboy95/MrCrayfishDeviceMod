package hu.gpsoft.device.block;
...
import hu.gpsoft.device.object.Bounds;
import hu.gpsoft.device.tileentity.TileEntityLaptop;
...
public class BlockLaptop extends BlockDevice.Colored
{
	...
	public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean p_185477_7_)
	{
		...
		if(tileEntity instanceof TileEntityLaptop)
		{
			...
			if(laptop.isOpen())
			{
				addCollisionBoxToList(pos, entityBox, collidingBoxes, BODY_OPEN_BOX);
				addCollisionBoxToList(pos, entityBox, collidingBoxes, SCREEN_BOXES[state.getValue(FACING).getHorizontalIndex()]);
			}
			else
			{
				addCollisionBoxToList(pos, entityBox, collidingBoxes, BODY_CLOSED_BOX);
			}
			return;
		}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		if(tileEntity instanceof TileEntityLaptop)
		{
			...
			else
			{
				...
				{
					playerIn.openGui(GPSoft.instance, Laptop.ID, worldIn, pos.getX(), pos.getY(), pos.getZ());
				}
			}
		}
		return true;

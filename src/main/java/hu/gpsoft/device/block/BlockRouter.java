...
import hu.gpsoft.device.network.task.MessageSyncBlock;
import hu.gpsoft.device.object.Bounds;
import hu.gpsoft.device.tileentity.TileEntityRouter;
...
public class BlockRouter extends BlockDevice.Colored
{
    ...
    public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean p_185477_7_)
    {
        if(state.getValue(VERTICAL))
        {
            addCollisionBoxToList(pos, entityBox, collidingBoxes, BODY_VERTICAL_BOUNDING_BOX[state.getValue(FACING).getHorizontalIndex()]);
        }
        else
        {
            addCollisionBoxToList(pos, entityBox, collidingBoxes, BODY_BOUNDING_BOX[state.getValue(FACING).getHorizontalIndex()]);
        }
    }

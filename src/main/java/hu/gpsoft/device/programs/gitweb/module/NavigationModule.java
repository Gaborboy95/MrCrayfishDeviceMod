package hu.gpsoft.device.programs.gitweb.module;
...
public class NavigationModule extends Module
{
    ...
    {
        List<hu.gpsoft.device.api.app.component.Button> navButtons = createNavigationButtons(null, data);
        int currentRow = 0;
        int rowItemCount = 0;
        int remainingWidth = width - 6;
        for(int i = 0; i < navButtons.size(); i++)
        {
            hu.gpsoft.device.api.app.component.Button button = navButtons.get(i);
    ...
    public void generate(GitWebFrame frame, Layout layout, int width, Map<String, String> data)
    {
        ...
        List<hu.gpsoft.device.api.app.component.Button> navButtons = createNavigationButtons(frame, data);
        int currentRow = 0;
        int rowItemCount = 0;
        int remainingWidth = width - 6;
        for(int i = 0; i < navButtons.size(); i++)
    ...
    private List<hu.gpsoft.device.api.app.component.Button> createNavigationButtons(@Nullable GitWebFrame frame, Map<String, String> data)
    {
        List<hu.gpsoft.device.api.app.component.Button> navButtons = new ArrayList<>();
        for(int i = 0; i < 10; i++)
        {
    ...
    private hu.gpsoft.device.api.app.component.Button createButton(@Nullable GitWebFrame frame, int index, Map<String, String> data)
    {
        String label = GitWebFrame.parseFormatting(data.getOrDefault("item-label-" + index, ""));
        hu.gpsoft.device.api.app.component.Button button = new Button(0, 0, label);

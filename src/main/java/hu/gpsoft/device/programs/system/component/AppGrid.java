...
import hu.gpsoft.device.api.app.component.Image;
 ...
 * Author: MrCrayfish
 */
public class AppGrid extends hu.gpsoft.device.api.app.Component
{
    ...
    private Layout generateAppTile(AppEntry entry, int left, int top)
    {
        ...
        if(entry instanceof LocalEntry)
        {
            ...
            hu.gpsoft.device.api.app.component.Image image = new hu.gpsoft.device.api.app.component.Image(iconOffset, padding, 14 * 3, 14 * 3, localEntry.getInfo().getIconU(), localEntry.getInfo().getIconV(), 14, 14, 224, 224, Laptop.ICON_TEXTURES);
        ...
        else if(entry instanceof RemoteEntry)
        {
            ...
            hu.gpsoft.device.api.app.component.Image image = new hu.gpsoft.device.api.app.component.Image(iconOffset, padding, 14 * 3, 14 * 3, ApplicationAppStore.CERTIFIED_APPS_URL + "/assets/" + resource.getResourceDomain() + "/" + resource.getResourcePath() + "/icon.png");
            layout.addComponent(image);
        }

        String clippedName = RenderUtil.clipStringToWidth(entry.getName(), itemWidth - padding * 2);
        hu.gpsoft.device.api.app.component.Label labelName = new hu.gpsoft.device.api.app.component.Label(clippedName, itemWidth / 2, 50);
        labelName.setAlignment(hu.gpsoft.device.api.app.Component.ALIGN_CENTER);
        layout.addComponent(labelName);

        String clippedAuthor = RenderUtil.clipStringToWidth(entry.getAuthor(), itemWidth - padding * 2);
        hu.gpsoft.device.api.app.component.Label labelAuthor = new hu.gpsoft.device.api.app.component.Label(clippedAuthor, itemWidth / 2, 62);
        ...
        {
            hu.gpsoft.device.api.app.component.Image certifiedIcon = new hu.gpsoft.device.api.app.component.Image(15, 38, Icons.VERIFIED);
        ...
        if(entry instanceof LocalEntry)
        {
            ...
            {
                hu.gpsoft.device.api.app.component.Image installedIcon = new hu.gpsoft.device.api.app.component.Image(itemWidth - 10 - 15, 38, Icons.CHECK);
    ...
    private void reloadIcons(Layout layout)
    {
        layout.components.forEach(component ->
        {
            ...
            else if(component instanceof hu.gpsoft.device.api.app.component.Image)
            {

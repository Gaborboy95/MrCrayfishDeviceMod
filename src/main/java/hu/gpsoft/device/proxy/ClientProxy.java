package hu.gpsoft.device.proxy;
...
import hu.gpsoft.device.DeviceConfig;
...
import hu.gpsoft.device.core.client.ClientNotification;
import hu.gpsoft.device.object.AppInfo;
import hu.gpsoft.device.programs.system.SystemApplication;
import hu.gpsoft.device.tileentity.*;
import hu.gpsoft.device.tileentity.render.*;
...
public class ClientProxy extends CommonProxy implements IResourceManagerReloadListener
{
        ...
        for(AppInfo info : ApplicationManager.getAllApplications())
        {
            ...
            try
            {
                ...
                if(input != null)
                {
                    ...
                    {
                        GPSoft.getLogger().error("Incorrect icon size for " + identifier.toString() + " (Must be 14 by 14 pixels)");
                        continue;
                    }
                ...
                {
                    GPSoft.getLogger().error("Icon for application '" + identifier.toString() +  "' could not be found at '" + path + "'");
                }
            }
            catch(Exception e)
            {
                GPSoft.getLogger().error("Unable to load icon for " + identifier.toString());
            }
        }
    ...
    public boolean registerPrint(ResourceLocation identifier, Class<? extends IPrint> classPrint)
    {
        try
        {
            ...
            {
                GPSoft.getLogger().error("The print renderer '" + classRenderer.getName() + "' is missing an empty constructor and could not be registered!");
                return false;
            }
            return true;
        }
        ...
        {
            GPSoft.getLogger().error("The print '" + classPrint.getName() + "' is missing an empty constructor and could not be registered!");
        }
        return false;
    }
